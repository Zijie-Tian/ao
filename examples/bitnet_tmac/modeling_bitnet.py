...
from utils_quant import BitLinearTMAC 
...
class BitnetMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = BitLinearTMAC(
            self.hidden_size, self.intermediate_size, bias=False, 
            weight_bits=config.weight_bits, input_bits=config.input_bits, 
        )
        self.up_proj = BitLinearTMAC(
            self.hidden_size, self.intermediate_size, bias=False, 
            weight_bits=config.weight_bits, input_bits=config.input_bits, 
        )
        self.down_proj = BitLinearTMAC(
...
class BitnetAttention(nn.Module):
    ...
    def __init__(self, config: BitnetConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = BitLinearTMAC(
            self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias, 
            weight_bits=config.weight_bits, input_bits=config.input_bits, 
        )
        self.k_proj = BitLinearTMAC(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias, 
            weight_bits=config.weight_bits, input_bits=config.input_bits, 
        )
        self.v_proj = BitLinearTMAC(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias, 
            weight_bits=config.weight_bits, input_bits=config.input_bits, 
        )
        self.o_proj = BitLinearTMAC(
